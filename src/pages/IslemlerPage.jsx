...
const predefinedRanges = [
  ...
  },
  {
    label: 'Bu Hafta',
    range: () => ({
      startDate: startOfWeek(new Date(), { locale: tr }),
      endDate: endOfWeek(new Date(), { locale: tr }),
    }),
  },
  {
    label: 'Bu Ay',
  ...
  },
  {
    ...
    range: () => ({
      ...
      endDate: endOfMonth(addDays(startOfMonth(new Date()), -1)),
    }),
  },
];
...
function IslemlerPage() {
  const {
    ...
    birlesikFiltreKategori,
    setBirlesikFiltreKategori,
    birlesikSiralamaKriteri,
    setBirlesikSiralamaKriteri,
    birlesikFiltreHesap,
    setBirlesikFiltreHesap,
    tarihAraligi,
    setTarihAraligi,
    aramaMetni,
    setAramaMetni,
  ...
  // Silme modalları için state
  const [topluSilModalAcik, setTopluSilModalAcik] = useState(false);
  const [tekliSilModalAcik, setTekliSilModalAcik] = useState(false);
  const [silinmekIstenenIslem, setSilinmekIstenenIslem] = useState(null);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { aciklama, tutar, kategori, tarih, hesapId } = formVerisi;

    if (!aciklama || !tutar || !kategori || !hesapId || parseFloat(tutar) <= 0)
      return toast.error('Lütfen tüm alanları doğru doldurun.');
  ...
  const handleDuzenleBaslat = (islem) => {
    if (islem.tip === ISLEM_TURLERI.TRANSFER) {
      toast('Transferler burada düzenlenemiyor.', { icon: 'ℹ️' });
      return;
    }
  ...
  const handleIslemKopyala = (islem) => {
    if (islem.tip === ISLEM_TURLERI.TRANSFER) {
      toast('Transferler kopyalanamıyor.', { icon: 'ℹ️' });
      return;
    }
    ...
    formRef.current?.scrollIntoView({ behavior: 'smooth' });
    toast.success(
      `'${islem.aciklama}' kopyalandı. Tarihi kontrol edip ekleyebilirsiniz.`
    );
  };
  ...
  const handleTumunuSec = () => {
    if (secilenIslemler.length === birlesikIslemler.length) {
      ...
      setSecilenIslemler(
        birlesikIslemler.map((islem) => ({ id: islem.id, tip: islem.tip }))
      );
    }
  };

  // Seçilenleri Sil butonuna basılınca: sadece modal aç
  const handleSecilenleriSil = () => {
    if (secilenIslemler.length === 0) return;
    setTopluSilModalAcik(true);
  };

  const handleTopluSilOnayla = () => {
    if (secilenIslemler.length === 0) return;
    handleTopluSil(secilenIslemler);
    setTopluSilModalAcik(false);
    setSecimModu(false);
    setSecilenIslemler([]);
  };

  const handleTopluSilIptal = () => {
    setTopluSilModalAcik(false);
  };

  // Tekli silme için
  const handleTekliSilBaslat = (islem, e) => {
    e.stopPropagation();
    setSilinmekIstenenIslem(islem);
    setTekliSilModalAcik(true);
  };

  const handleTekliSilOnayla = () => {
    if (!silinmekIstenenIslem) return;
    handleTopluSil([{ id: silinmekIstenenIslem.id, tip: silinmekIstenenIslem.tip }]);
    setTekliSilModalAcik(false);
    setSilinmekIstenenIslem(null);
  };

  const handleTekliSilIptal = () => {
    setTekliSilModalAcik(false);
    setSilinmekIstenenIslem(null);
  };
  ...
  const formatTarihAraligi = () => {
    ...
    return `${format(startDate, 'd MMM yyyy', { locale: tr })} - ${format(
      endDate,
      'd MMM yyyy',
      { locale: tr }
    )}`;
  ...
  if (!hesaplar || !birlesikIslemler)
    return <div className="sayfa-yukleniyor">Yükleniyor...</div>;
  ...
  const filtrelenmisToplamGelir = birlesikIslemler
    .filter((i) => i.tip === 'gelir')
    .reduce((acc, i) => acc + i.tutar, 0);
  const filtrelenmisToplamGider = birlesikIslemler
  ...
  return (
    <>
          ...
          >
            ...
            >
              ...
              <div className="modal-actions">
                <button onClick={handleTakvimIptal} className="cancel-btn">
                  İptal
                </button>
                <button onClick={handleTakvimKaydet} className="primary-btn">
                  Tamam
                </button>
              </div>
            </motion.div>
      ...
      {/* Toplu silme onay modali */}
      <AnimatePresence>
        {topluSilModalAcik && (
          <motion.div
            className="modal-overlay"
            onClick={handleTopluSilIptal}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <motion.div
              ...
              exit={{ y: -30, opacity: 0 }}
            >
              <h3>Seçilen işlemleri sil</h3>
              <p>
                <strong>{secilenIslemler.length}</strong> adet işlem kalıcı olarak
      ...
      {/* Tekli silme onay modali */}
      <AnimatePresence>
        {tekliSilModalAcik && (
          <motion.div
            className="modal-overlay"
            onClick={handleTekliSilIptal}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            ...
            >
              <h3>İşlemi sil</h3>
              <p>
                <strong>{silinmekIstenenIslem?.aciklama || 'Bu işlem'}</strong> kalıcı
      ...
      <div className="card" ref={formRef}>
        <div className="islem-tipi-secici">
          <button
            onClick={() => handleTabClick(ISLEM_TURLERI.GIDER)}
            className={aktifIslemTipi === ISLEM_TURLERI.GIDER ? 'aktif' : ''}
          >
            Gider
          </button>
          <button
            onClick={() => handleTabClick(ISLEM_TURLERI.GELIR)}
            className={aktifIslemTipi === ISLEM_TURLERI.GELIR ? 'aktif' : ''}
          >
            Gelir
          </button>
          {/* Transfer sekmesi kaldırıldı */}
        </div>
      ...
      <div className="card">
        <div className="liste-baslik">
          ...
          <div className="liste-eylemler">
            ...
            <button
              ...
              className="filtre-ac-btn"
            >
              <FaFilter /> Diğer Filtreler
            </button>

            {/* Seç butonu sadece seçim modu KAPALIYKEN görünsün */}
            {!secimModu && (
              <button
                onClick={handleSecimModuToggle}
                className="filtre-ac-btn"
              >
                Seç
              </button>
            ...
            >
              <div className="filtre-grid">
                <div className="kontrol-grubu">
                  ...
                  <select
                    value={birlesikFiltreTip}
                    onChange={(e) => setBirlesikFiltreTip(e.target.value)}
                  >
                    <option value={ISLEM_TURLERI.TUMU}>Tümü</option>
                    <option value={ISLEM_TURLERI.GELIR}>Gelir</option>
                    <option value={ISLEM_TURLERI.GIDER}>Gider</option>
                    <option value={ISLEM_TURLERI.TRANSFER}>Transfer</option>
                  </select>
                </div>
                <div className="kontrol-grubu">
                  <label>Kategoriye Göre Filtrele:</label>
                  <select
                    value={birlesikFiltreKategori}
                    onChange={(e) =>
                      setBirlesikFiltreKategori(e.target.value)
                    }
                  >
                ...
                <div className="kontrol-grubu">
                  ...
                  <select
                    value={birlesikFiltreHesap}
                    onChange={(e) =>
                      setBirlesikFiltreHesap(e.target.value)
                    }
                  >
                    <option value="Tümü">Tümü</option>
                    {tumHesaplar.map((hesap) => (
                ...
                <div className="kontrol-grubu">
                  <label>Sırala:</label>
                  <select
                    value={birlesikSiralamaKriteri}
                    onChange={(e) =>
                      setBirlesikSiralamaKriteri(e.target.value)
                    }
                  >
                    <option value={SIRALAMA_KRITERLERI.TARIH_YENI}>
                      Tarihe Göre (En Yeni)
                    </option>
                    <option value={SIRALAMA_KRITERLERI.TARIH_ESKI}>
                      Tarihe Göre (En Eski)
                    </option>
                    <option value={SIRALAMA_KRITERLERI.TUTAR_ARTAN}>
                      Tutara Göre (Artan)
                    </option>
                    <option value={SIRALAMA_KRITERLERI.TUTAR_AZALAN}>
                      Tutara Göre (Azalan)
                    </option>
                  </select>
                </div>
        ...
        <AnimatePresence>
          {secimModu && (
            <motion.div
              className="secim-eylem-cubugu"
              initial={{ height: 0, opacity: 0 }}
              animate={{ height: 'auto', opacity: 1 }}
              exit={{ height: 0, opacity: 0 }}
            >
              <div className="secim-sol">
                <button
                  onClick={handleTumunuSec}
                  className="secim-eylem-btn"
                >
                  {secilenIslemler.length === birlesikIslemler.length ? (
                    <FaCheckSquare />
                  ) : (
                    <FaSquare />
                  )}
              ...
              <div className="secim-sag">
                <button
                  onClick={handleSecilenleriSil}
                  className="danger-btn"
                  disabled={!birseySeciliMi}
                >
        ...
        <div className="sayfa-ici-ozet">
          <div className="ozet-kalem">
            ...
            <span className="gelir-renk">
              {formatCurrency(filtrelenmisToplamGelir)}
            </span>
          </div>
          <div className="ozet-kalem">
            <span>Toplam Gider:</span>
            <span className="gider-renk">
              {formatCurrency(filtrelenmisToplamGider)}
            </span>
          </div>
          <div className="ozet-kalem">
            <span>Net Durum:</span>
            <span
              className={
                netDurum >= 0 ? 'gelir-renk' : 'gider-renk'
              }
            >
        ...
        <ul
          className={`islem-listesi-yeni ${
            secimModu && birseySeciliMi ? 'secim-modu-aktif' : ''
          }`}
        >
          <AnimatePresence>
            {birlesikIslemler.map((islem) => {
              const isTransfer = islem.tip === ISLEM_TURLERI.TRANSFER;
              const gonderenHesap = isTransfer
                ? hesaplar.find((h) => h.id === islem.gonderenHesapId)
                : null;
              const aliciHesap = isTransfer
                ? hesaplar.find((h) => h.id === islem.aliciHesapId)
                : null;
              const hesap = !isTransfer
                ? tumHesaplar.find((h) => h.id === islem.hesapId)
                : null;
              const isSelected = secilenIslemler.some(
                (item) => item.id === islem.id
              );
              ...
              return (
                <motion.li
                  ...
                  className={`islem-karti-${islem.tip} ${
                    secimModu ? 'secilebilir' : ''
                  } ${isSelected ? 'secili' : ''}`}
                >
                  {secimModu && (
                  ...
                  <div className="islem-ikon">
                    {getCategoryIcon(isTransfer ? 'Transfer' : islem.kategori)}
                  </div>

                  <div className="islem-orta">
                    <span className="islem-aciklama">
                      {isTransfer
                        ? islem.aciklama || 'Hesaplar Arası Transfer'
                        : islem.aciklama}
                    </span>
                  ...
                  <div className="islem-sag">
                    ...
                    {!secimModu && (
                      <div className="buton-grubu">
                        {/* Transferlerde kopyala / düzenle yok */}
                        {!isTransfer && (
                          <>
                            <button
                              onClick={() => handleIslemKopyala(islem)}
                              className="icon-btn"
                              title="Bu işlemi kopyala"
                            >
                              <FaCopy />
                            </button>
                            <button
                              onClick={() => handleDuzenleBaslat(islem)}
                        ...
                        <button
                          onClick={(e) => handleTekliSilBaslat(islem, e)}
