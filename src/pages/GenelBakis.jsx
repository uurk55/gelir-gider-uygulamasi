...
  const {
    seciliYil, seciliAy,
    hesapDurumlari, // <-- EKSİK OLAN PARÇA BUYDU!
...
        <div className="hesap-kartlari-konteyner">
    <h2>Hesap Bakiyeleri</h2>
    <div className="hesap-kartlari">
        {hesapDurumlari.map(hesap => (
            <div key={hesap.id} className="hesap-karti">
                <span className="hesap-adi">{hesap.ad}</span>
                <span className={`hesap-bakiye ${hesap.bakiye >= 0 ? 'gelir-renk' : 'gider-renk'}`}>
                    {hesap.bakiye.toFixed(2)} ₺
                </span>
            </div>
        ))}
    </div>
...
      {/* 3. GELİR VE GİDER ANALİZ IZGARASI */}
